...
import { getAnsBoundary, createPuzzle, validate } from './CombineGame.helpers';
...
import type { Question, Difficulty, GameStatus } from './types';
...
function CombineGame({
	...
	const [puzzle, setPuzzle] = React.useState<string[]>([]);
	const [missingPuzzleIndex, setMissingPuzzleIndex] = React.useState<
		undefined | number
	>(undefined);
	...
	const handleAddNum = React.useCallback(function (num: number) {
		setGuess(g => {
	...
	function handleGuess(event: React.PointerEvent<HTMLButtonElement>) {
	...
	function handleFinish(event: React.PointerEvent<HTMLButtonElement>) {
		event.preventDefault();
	...
	function handleRestart(event: React.PointerEvent<HTMLButtonElement>) {
		event.preventDefault();
		moveToNextQuestion();
		setQuestionIndex(0);
		setCountdown(3);
