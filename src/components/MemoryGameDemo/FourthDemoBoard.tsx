...
function FourthDemoBoard(
	{ mode }: { mode: Mode },
	topLeftAnchorRef: React.ForwardedRef<HTMLButtonElement>,
) {
	const pieceRef = React.useRef(new Map());
	...
	function setUpNode(node: HTMLButtonElement | null, index: number) {
		const map = getMap();
		if (node) {
			map.set(index, node);
		} else {
