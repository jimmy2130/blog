...
import { type SecondDemoPiece } from './types';

const INITIAL_GAME: SecondDemoPiece[] = [
	{ state: 'cover', id: 'abc' },
	{ state: 'cover', id: 'def' },
];

// 從mdx指定要成功的Demo還是失敗的Demo
function SecondDemo({ match = 'fail' }: { match?: 'fail' | 'success' }) {
	...
	function reveal(id: string) {
		...
		const nextGame = [...game];
		const nextPiece: SecondDemoPiece = { ...piece, state: 'active' };
	...
	React.useEffect(() => {
		function showResult(action: string) {
			const nextGame: SecondDemoPiece[] = game.map(piece => {
				return {
					...piece,
